...
public class Blaster extends MainWeapon {

	public Blaster() {
		super("bullet_blaster", WeaponType.Blaster);
	}
	...
	public void doOnInteract(PlayerInteractEvent e, PlayerData pd, Player pe) {
		...
		if(pd.getTask() == null || pd.getTask().isCancelled()) {
			BukkitRunnable task = new BlasterRunnable(pe.getName(),this);
	...
	public void shoot(PlayerData data) {
		...
		MainGame.sync(() -> {
			...
			BukkitRunnable task = new BlasterBulletRunnable(player.getName(),snowball,this);
