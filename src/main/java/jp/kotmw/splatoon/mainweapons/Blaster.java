...
import jp.kotmw.splatoon.mainweapons.threads.BlasterBulletRunnable;
import jp.kotmw.splatoon.mainweapons.threads.BlasterRunnable;
...
public class Blaster implements Listener {

	public static final String bulletname="bullet_blaster";
	...
	public void onInteract(PlayerInteractEvent e) {
		...
		if(DataStore.getWeapondata(data.getWeapon()).getType() != WeaponType.Blaster)
			return;
		if(data.isAllCancel()
				|| item == null
		...
		if(data.getTask() == null || data.getTask().isCancelled()) {
			BukkitRunnable task = new BlasterRunnable(player.getName());
		...
		&&(e.getEntity() instanceof Player || e.getEntity() instanceof Creeper)) {
			...
			if(e.getEntity() instanceof Player) {
				...
				if (!DataStore.hasPlayerData(shooter.getName())
					...
					return;
				player.playSound(ball.getLocation(),Sound.BLOCK_ANVIL_PLACE,SoundCategory.PLAYERS,0.6f,1.2f);
			}
			//System.out.println("call4");
			WeaponData data = DataStore.getWeapondata(DataStore.getPlayerData(shooter.getName()).getWeapon());
			if(data.getType() != WeaponType.Blaster)
				return;
			//System.out.println("call5");

			e.setDamage(data.getDamage()*getDecayRate(ball,data));
			shooter.playSound(ball.getLocation(),Sound.BLOCK_ANVIL_PLACE,SoundCategory.PLAYERS,0.6f,1.2f);
	...
	public static void shoot(PlayerData data) {
		...
		MainGame.sync(() -> {
			...
			BukkitRunnable task = new BlasterBulletRunnable(player.getName(),snowball);
		...
		data.setRecoilTick(weapon.getRecoilTick());
